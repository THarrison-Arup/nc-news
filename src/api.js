...
export const fetchArticleBySearch = async (id, author, title, sort_by) => {
  if (author || title) {
    const { data } = await axios.get(`${BASE_URL}/articles/${id}`, {params: {author: author, title: title, sort_by: sort_by}});
    return data.articles;
  } else {
...
export const deleteArticleComment = async (commentId, articleId) => {
  console.log(commentId,'<comment', articleId,'<article')
  const { data } = await axios.delete(`${BASE_URL}/comments/${commentId}`)
  .then(fetchCommentsById(articleId))
  return data;
};